...
describe('API Integration Tests', () => {
  beforeEach(() => {
    ...
    db.exec(`
      ...
      DELETE FROM labels;
      DELETE FROM lists WHERE is_default = 0;
    `)
  })
  ...
  it('should delete a task via API', async () => {
    ...
    const purgeRes = await fetch(`http://localhost:3000/api/tasks/${created.id}?purge=true`, {
      method: 'DELETE',
    })
    expect(purgeRes.status).toBe(200)

    const getPurgedRes = await fetch(`http://localhost:3000/api/tasks/${created.id}`)
    expect(getPurgedRes.status).toBe(404)
  })
